...
class SzamlaAgent {
     ...
     */
    private function __construct(string $apiKey, bool $downloadPdf = true, string $aggregator = '', int $logLevel = Log::LOG_LEVEL_DEBUG, bool $singleton)
    {
        ...
        $this->setting->singleton = $singleton;
    }
     ...
     * Singleton üzemmód esetén csak egy példány létezik az adott api kulccsal
     *
     * @param string $apiKey Számla Agent kulcs
     * @param bool $downloadPdf elkeszült bizonylat letöltése
     * @param string $aggregator webáruházat futtató motor neve
     * @param int $logLevel naplózási szint
     * @param bool $singleton singleton üzemmód
     * @return SzamlaAgent
     * @throws SzamlaAgentException
     */
    public static function create(string $apiKey, bool $downloadPdf = true, string $aggregator = '', int $logLevel = Log::LOG_LEVEL_DEBUG, bool $singleton = true): SzamlaAgent
    {
        $agent = null;
        ...
        if ($singleton) {
            ...
            if (isset(self::$agents[$index])) {
                ...
                $agent = new self($apiKey, $downloadPdf, $aggregator, $logLevel, $singleton);
    ...
    {
        $message = $this->setting->singleton ? $message : '[' . spl_object_hash($this->setting) . '] - ' . $message;

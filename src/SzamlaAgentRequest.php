...
class SzamlaAgentRequest
{
    ...
    {
        $prefix = $this->agent->setting->singleton ? 'response' : 'response-' . spl_object_hash($this->agent);
        $fileName = SzamlaAgentUtil::getXmlFileName($prefix, $this->getXmlName());
    ...
    private function makeCurlCall(): array
    {
        ...
        if (($agent->setting->xmlFileSave && $agent->setting->requestXmlFileSave)) {
            ...
            $prefix = $this->agent->setting->singleton ? 'request' : 'request-' . spl_object_hash($this->agent);
            $xmlContent = 'data://application/octet-stream;base64,' . base64_encode($this->getXmlData());
            $fileName = SzamlaAgentUtil::getXmlFileName($prefix, $this->getXmlName(), $this->entity);
